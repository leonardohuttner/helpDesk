...
public class ChamadoDB {
    ...
    public void save(Chamado chamado) {
        try {
            PreparedStatement ps = conexao.prepareStatement("INSERT INTO atendimento (titulo, descricao, id_cliente, data_hora_abertura, id_responsavel, status) values (?,?,?,?,?,?)");
            ps.setString(1, chamado.getTitulo());
            ps.setString(2, chamado.getDescricao());
            ps.setString(3, chamado.getId());
            ps.setString(4, chamado.getSexo());
            ps.setString(5, chamado.getEndereco());
            ps.setString(6, chamado.getCpf());
